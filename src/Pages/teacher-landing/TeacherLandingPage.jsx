...
const TeacherLandingPage = () => {
  ...
  const askQuestion = () => {
    ...
    if (!socket) {
      showError("Connection error: Socket not initialized.");
      return;
    }
    ...
    try {
      ...
      socket.emit("createPoll", pollData, (response) => {
        setIsCreating(false);
        ...
        if (response?.success) {
          ...
          const errorMsg = response?.message || "Failed to create poll.";
          showError(errorMsg);
        }
      });
      ...
      setIsCreating(false);
      const errorMsg = err instanceof Error ? err.message : "Error creating poll.";
      showError(errorMsg);
    }
  };
  ...
  return (
    <>
      ...
      <div className="container my-4 w-75 ms-5">
        <button className="btn btn-sm intervue-btn mb-3" onClick={() => navigate("/")}>
